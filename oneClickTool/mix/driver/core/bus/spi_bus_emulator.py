...
from axi4_lite_def import PLSPIDef
...
class SPIBusEmulator(object):
    ...
    def _load_plugin(self):
        ...
        try:
            ...
            raise SPIBusException('%s load plugin failed' % (self._dev_name))
        ...
        if self._plugin is None:
            raise SPIBusException("%s can not find plugin %s" % (self._dev_name, plugin_lower_name))
...
class PLSPIBusEmulator(SPIBusEmulator):
    pass


class PSSPIBusEmulator(SPIBusEmulator):
    def __init__(self, dev_name):
        self._dev_name = dev_name
        SPIBus.__init__(dev_name, 0)
