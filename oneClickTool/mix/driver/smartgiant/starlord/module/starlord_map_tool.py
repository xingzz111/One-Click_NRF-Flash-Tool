...
from mix.driver.smartgiant.starlord.module.starlord_map import StarLord
...
class MIX_StarLord_SGDebuger(cmd.Cmd):
    ...
    def do_post_power_on_init(self, line):
        ...
        self.starlord.post_power_on_init(eval(line))
        print("Done.")

    @handle_errors
    def do_reset(self, line):
        '''reset timeout'''
        self.starlord.reset(eval(line))
        print("Done.")

    @handle_errors
    def do_pre_power_down(self, line):
        '''pre_power_down timeout'''
        self.starlord.pre_power_down(eval(line))
        print("Done.")

    @handle_errors
    def do_get_driver_version(self, line):
        '''get_driver_version'''
        result = self.starlord.get_driver_version()
        print("Result:")
        print(result)
