...
from mix.driver.smartgiant.omega.module.dmm005004_map import DMM005004
...
class OmegaDebuger(cmd.Cmd):
    ...
    def do_post_power_on_init(self, line):
        ...
        self.omega.post_power_on_init(eval(line))
        print("Done.")

    @handle_errors
    def do_reset(self, line):
        '''reset timeout'''
        self.omega.reset(eval(line))
        print("Done.")

    @handle_errors
    def do_pre_power_down(self, line):
        '''pre_power_down timeout'''
        self.omega.pre_power_down(eval(line))
        print("Done.")

    @handle_errors
    def do_get_driver_version(self, line):
        '''get_driver_version'''
        result = self.omega.get_driver_version()
        print("Result:")
        print(result)
    ...
    @handle_errors
    def do_mcp4725_voltage_output(self, line):
        '''mcp4725_voltage_output
        ...
        if '?' == line:
            print(get_function_doc(self))
            return None
        self.omega.mcp4725_voltage_output(eval(line))
        print('Done.')

    @handle_errors
    def do_dds_mode_select(self, line):
        '''dds_mode_select
        select dds range (200/500)mv
        mode: '200mv', '500mv'
        eg: dds_mode_select '200mv' '''
        if '?' == line:
            print(get_function_doc(self))
            return None
        self.omega.dds_mode_select(eval(line))
        print('Done.')
