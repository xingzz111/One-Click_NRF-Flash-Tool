...
from mix.driver.smartgiant.starlordii.module.audio005001_map import Audio005001
...
class MIX_Audio005001_Debuger(cmd.Cmd):
    prompt = 'audio005001>'
    intro = 'Xavier audio005001 debug tool, Type help or ? to list commands.\n'
    ...
    def do_write_calibration_cell(self, line):
        ...
        self.audio005001.write_calibration_cell(unit_index, gain, offset, threshold)
        print("Done.")
    ...
    def do_read_calibration_cell(self, line):
        ...
        unit_index = int(line)
        ret = self.audio005001.read_calibration_cell(unit_index)
    ...
    def do_erase_calibration_cell(self, line):
        ...
        self.audio005001.erase_calibration_cell(eval(line))
        print("Done.")

    @handle_errors
    def do_set_cal_mode(self, line):
        '''set_cal_mode <raw/cal>'''
        self.audio005001.set_calibration_mode(eval(line))
        print("Done.")

    @handle_errors
    def do_get_cal_mode(self, line):
        '''get_cal_mode'''
        result = self.audio005001.get_calibration_mode()
        print("Result:")
        print(result)
    ...
    def do_post_power_on_init(self, line):
        ...
        self.audio005001.post_power_on_init(eval(line))
        print("Done.")

    @handle_errors
    def do_reset(self, line):
        '''reset timeout'''
        self.audio005001.reset(eval(line))
        print("Done.")

    @handle_errors
    def do_pre_power_down(self, line):
        '''pre_power_down timeout'''
        self.audio005001.pre_power_down(eval(line))
        print("Done.")

    @handle_errors
    def do_get_driver_version(self, line):
        '''get_driver_version'''
        result = self.audio005001.get_driver_version()
        print("Result:")
        print(result)
    ...
    def do_enable_upload(self, line):
        '''enable_upload'''
        self.audio005001.enable_upload()
        print("Done.")

    @handle_errors
    def do_disable_upload(self, line):
        '''disable_upload'''
        self.audio005001.disable_upload()
        print("Done.")
    ...
    def do_measure(self, line):
        ...
        line = line.replace(' ', ',')
        info = self.audio005001.measure(*list(eval(line)))
        print("Result:")
    ...
    def do_enable_output(self, line):
        ...
        line = line.replace(' ', ',')
        self.audio005001.enable_output(*list(eval(line)))
        print("Done.")

    @handle_errors
    def do_disable_output(self, line):
        '''disable_output'''
        self.audio005001.disable_output()
        print("Done.")

    @handle_errors
    def do_read_temperature(self, line):
        '''read_temperature'''
        result = self.audio005001.get_temperature()
        print("Result:")
        print(result)
...
def create_beast_dbg(ip_name, i2c_bus_name):
    audio005001_dbg = MIX_Audio005001_Debuger()
    ...
    audio005001_dbg.audio005001 = Audio005001(i2c=i2c_bus_1, ipcore=mix_aut5_r)
    return audio005001_dbg
